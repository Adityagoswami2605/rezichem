...
/****************************************
	Database Connection
*****************************************/
$link_id = mysqli_connect($db_host,$db_user,$db_password,$db_name);
mysqli_select_db($db_name,$link_id);
...
function get_array($result_id) {
	$records = @mysqli_fetch_assoc($result_id);
	if($records){
...
function upload_file($files,$new_path)
{
	$file = $files['name'];
	$uploadedfile = $files['tmp_name'];
	if ($file) 
	{
		 $filename = str_replace(' ','-',strtolower(stripslashes($file)));
		 ...
		 $new_path = $new_path.$filename;
	 	 ...
	 	 move_uploaded_file($uploadedfile,$new_path);
