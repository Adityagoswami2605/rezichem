...
{
	$sql = "SELECT COUNT(*) FROM category where category_status =1 ";
	$results = get_row($sql);
	return $results['COUNT(*)'];
}
function get_categories_limit($limit='1')
{
	$sql = "select * from category where category_status = 1 limit " .$limit;
	$results = get_results($sql);
	return $results;
}
...
{
	$sql = "select category_id,category_name from category where category_status = 1 AND category_parent_id=".$parent;
	$results = get_results($sql);
	return $results;
}
...
{
	$sql = "select category_id from category where category_name='".$name."' and category_parent_id = 0";
	$results = get_row($sql);
...
function update_category_commision($category_id,$value)
{
	$sql = 'UPDATE category SET category_commision = '.$value.' WHERE `category`.`category_id` ='.$category_id.'';
	$results = get_results($sql);
	return $results;
}
function get_cat_details($id)
{
	$query = mysqli_query("SELECT * FROM category where category_id=".$id);
...
function get_category_by_id($id){
	...
	return $results1;*/
	$query = mysqli_query("SELECT * FROM category where category_parent_id=".$id);
...
{
	//$query = mysqli_query("SELECT *  FROM products p,product_categories pc WHERE pc.pc_product_id = p.product_id and p.product_id =".$id." limit 0,30");
...
function category_sub_cat_by_id($id){
	...
	for($i=0;$i<count($details_arr);$i++)
	{
		...
		echo $details_arr[$i]['category_name']."<br />";
	}
}
...
function get_category_detail_byslug($slug)
{
