...
if(count($_POST) > 0)
{
	if(trim($_POST['clients_name']) == '') 
		{
		$clients_name_error = '<label class="alert-danger fade in">Enter Name</label>';	
		$error = 1;
	}
	$image_count = count($_FILES['clients_image']['name']);
	if($image_count <= 0 || $_FILES['clients_image']['name'][0] == '')
	{
		$clients_image_error = '<label class="alert-danger fade in">This field is required.</label>';	
		$error = 1;
	}
	if($error == 1)
	{
	...
	else
	{
			...
			$_POST['clients_created_date'] = $current_date;
			
			
			$insert_id = insert_data('clients',$_POST);
			if($insert_id)
			{
                    ...
                    <section class="panel">
                        ...
                        <div class="panel-body">
                            <div class="position-center">
								...
								<div class="form-group">
                                    <label>Name (*)</label>
                                    <input type="text" class="form-control"  placeholder="Enter name" name="clients_name" value="<?=$_POST['clients_name']?>" />
						...
						<!--	<div class="form-group">
                            ...
                            <div class="form-group">
								...
								<div class="radio">
									<label>
										<input type="radio" name="clients_status" value="1" checked="checked" >
										Publish
									</label>
								</div>
								<div class="radio">
									<label>
										<input  type="radio" name="clients_status" value="2">
										Draft
									</label>
								</div>
                             </div>
